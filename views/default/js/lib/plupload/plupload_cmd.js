define('plupload', function(require) {
...
//    var alertMsg = window.alert;
    ...
    var opts = $.pluploadOpts = {
        template: {
            images: imagesTpl,
            attachments: attachmentsTpl
        },
        block_when_uploading: false,
        ...
        upload_limit: 10,
        // 当上传达到限制的时候，覆盖最先上传的元素
        upload_limit_override_item: false,
        ...
        responseParser: function(res) {
            if (res && res.success || res.code == 1000) {
                if (res.filename || res.data.filename) {
                    return {
    ...
    $.fn.plupload = function(settings) {
        ...
        settings = settings || {};
        settings.multipart_params = settings.multipart_params || {};
        settings.multipart_params.web_csrf_token = $('#web_csrf_token').val();
        settings.multipart_params.token = settings.token;
        ...
        if (settings.block_when_uploading) {
            self.addClass('block_when_uploading');
        }
        ...
        pluploader.bind('Error', function(up, error) {
            ...
            var $item = $('#uploadItem' + error.file.id);
        ...
        pluploader.bind('FilesAdded', function(up, files) {
            ...
            if (settings.upload_limit_override_item) {
                while (files.length > settings.upload_limit) {
                    var file = files.pop();
                    up.removeFile(file);
                }

                while (files.length > settings.upload_limit - uploaded) {
                    removeItem(items[0].path);
                    uploaded--;
                }
            }
        ...
        pluploader.bind('FileUploaded', function(up, file, res) {
            ...
            if (res.response) {
                ...
                if (r.path || r.id) {
                    var src = getViewPath(r.original_data ? r.original_data.url : r.path);
                    var $thumb = $item.find('.js_thumb');
                    if ($thumb.length) {
                        var $img = $('<img class="js-uploadimg" src="' + src + '">');
                        $thumb.append($img);
                    }

                    processItem($item, r.path, r.id, result, result.data);
        ...
        function addItem(filepath, id, name, originalData) {
            if (!filepath) {
                return;
            }
            ...
            var $thumb = $item.find('.js_thumb');
            if ($thumb.length) {
                var $img = $('<img class="js-uploadimg" src="' + getViewPath(getUrlFromOriginalData(originalData) || filepath) + '">');
                $thumb.append($img);
            }
            name = name || '附件';
            $item.find('.js_filename').attr('title', name).text(name);
            $list.append($item);
            processItem($item, filepath, id, null, originalData);
            settings.onitemschange.apply(self, [uploader, $list]);
        }
        ...
        function processItem($item, filepath, id, res, originalData) {
            $item.find('.js_link').attr({
                'href': getViewPath(getUrlFromOriginalData(originalData) || filepath),
                'data-path': filepath
            });
            ...
            if (items.length >= settings.upload_limit && !settings.upload_limit_override_item) {
                disable();
            }
        }
        ...
        function getParam(name) {
            return pluploader.settings.multipart_params[name];
        }
        ...
        function getViewPath(path) {
            if (isRelativePath(path)) {
                return settings.viewBasePath + path;
            } else {
                return path;
            }
        }

        function getUrlFromOriginalData(originalData) {
            if (originalData) {
                return originalData.url;
            }
        }
