define('lib/xiaomili/xiaomili.window.js', function(require) {
    var Weiboyi = require('lib/xiaomili/xiaomili.button.js');

    require('lib/xiaomili/xiaomili.tabctnr.js');
    ...
    (function ($, W, window, undefined) {
        ...
        var methods = {
            locate: function (position) {
                ...
                if (W.util.ie6 || !this.isPositionFixed) {
                    css.position = "absolute";
                    var gap = ($(window).height() - this.getHeight());
                    gap = gap > 0 ? gap : 0;
                    css.top = $(window).scrollTop() + gap / 2;
                } else {
                    css.position = "fixed";
                    css["margin-top"] = -(this.getHeight() / 2) + "px";
                }
            ...
            show: function () {
                ...
                if (self.modal !== false) {
                    ...
                    self.mask.show();
                    if ($.isNumeric(self.modal)) {
                        self.mask.css({
                            opacity: self.modal,
                            filter: "alpha(opacity=" + self.modal * 100 + ")"
                        });
                    }
                }
